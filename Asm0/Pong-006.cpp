...
void ROM::generateAssembly()
 {
   ...
   int start = getCodeLoc();
                        ...
                        LRA(735);     // A print later will tell you where this needs to jump to.
                        RET(0);
...
// Update the ball.
   ...
   // 0015      : Y velocity of the ball
                        ...
                        LDI(0xA);   // Load the frame counter: only update every 4 frames (7.5 frames / sec)
                        LDB(0);
                        DEC(0);     // Decrement counter
                        BRZ(0, 4);  // If the counter is zero, update
                        ...
                        LDI(0x12);  // X location of ball
                        LDB(0);
                        LDI(2);
                        SUB(0, 1);
                        NOP();
                        BR(0, 20); // If we are at column 2, else goto $$$
                        LDI(0x13); // Y location of the ball
                        LDI(0x10); // Y location of left paddle
                        LDB(1);    //     ball
                        LDB(1);    //     paddle
                        DEC(0);
                        ...
                        LDI(0x13);  // Reload Y : because we don't sign extend the change, this drops the carry out when change is -1
                        LDB(0);
                        BRP(0, 4);  // If Y is not zero, jump ahead to !!!
                        ...
                        SUB(0, 1);
                        BRP(0, 3);
                        LDI(0x15);  // If Y is 23 (!or more!) change the direction of Y change
                        NOT(15);
                        STB(0, 1);
                        ...
                        LDI(0x12);  // Reload X : because we don't sign extend the change, this drops the carry out when change is -1
                        LDB(0);
                        BRZ(0, 6); // If zero, then we are on the far left, and the right player has scored: jump to reset
                        LDI(78);
                        SUB(0, 1);
                        BRZN(0, 3); // If > 78, then we are on the far right, and the left player has scored: jump to reset
