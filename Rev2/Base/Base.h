...
/*
   The fake architecture is for a 48 pin DIP processor. It has 15 address pins and 16 data pins.
   ...
   The processor also has five pins to service eight levels of either maskable or non-maskable interrupts.
...
class Core : public ClockDevice
 {
   private:
      byte F; // The active layers: SDO, Int, NMI
      byte L; // The current layer
      word PC [4];     // The program counter
      word B  [4][16]; // The belt
      byte P  [4];     // The location in the belt's ring buffer of the next write
      word R     [16]; // The 16 scratch locations. Don't call them registers.
      MemoryController* M; // Our view into memory

      void clearBelt(int n);
      void dropResult(word r);
      word getBelt(word l) const;
